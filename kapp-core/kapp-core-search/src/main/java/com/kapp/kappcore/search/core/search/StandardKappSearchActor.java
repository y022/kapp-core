...
public class StandardKappSearchActor extends AbstractKappSearchActor {
    ...
    @Override
    public <R> R search(SearchParam searchParam, SearchCallBack<SearchResponse, R> callBack) throws SearchException {
        searchParam.startTime();
        SearchRequest searchRequest = createSearchRequest(searchParam);
        R r = searchParam.continueScroll()
                ? scroll(searchParam.getScrollId(), callBack)
                : doSearch(searchRequest, callBack);
        searchParam.endTime();
        return r;
    }

    @Override
    public <R> R scroll(String scrollId, SearchCallBack<SearchResponse, R> callback) throws SearchException {
        return scroll(new SearchScrollRequest(scrollId), callback);
    }
